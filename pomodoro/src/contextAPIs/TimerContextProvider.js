...
const TimerContextProvider = (props) => {
    ...
    switch (stateVariable.activeTab) {
      case 1: //for pomodoro
        ...
        if (stateVariable.onStart && stateVariable.pomo >= 0) {
          //when the pomodoro starts and them timer is set to more than 0sec
          ...
          //whem the timer is start but timer is set to 0
  ...
  const toggleTabsHandler = (index) => {
    //toggling between tabs and if toggled while a timer is running then resetting the running timer to it's initial state
  ...
  const setTimerHandler = (times) => {
    //setting the time in the timers via settings
